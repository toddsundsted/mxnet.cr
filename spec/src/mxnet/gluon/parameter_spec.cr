...
describe MXNet::Gluon::ParameterDict do
  ...
  describe "#save" do
    ...
    parameter_dict =
      MXNet::Gluon::ParameterDict.new(prefix: "name").tap do |parameter_dict|
        parameter_dict.get("test", shape: [3])
        parameter_dict.init(init: :ones)
      end
    data = Bytes[
  ...
  describe "#load" do
    ...
    it "loads the parameters" do
      File.open(temp, "w") do |file|
        file.write(data)
        file.close
        parameter_dict.load(temp)
        parameter_dict.get("test").data.should eq(MXNet::NDArray.ones([3], dtype: :float32))
      end
    end
    ...
    it "raises an error if parameter not present in file" do
      parameter_dict =
        MXNet::Gluon::ParameterDict.new(prefix: "name").tap do |parameter_dict|
          parameter_dict.get("foo", shape: [3])
        end
    ...
    it "raises an error if value not present in dict" do
      parameter_dict =
        MXNet::Gluon::ParameterDict.new(prefix: "name")
